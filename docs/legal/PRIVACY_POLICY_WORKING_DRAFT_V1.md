...
Version: 0.3.0 (Working Draft — Third Pass, post-ADR-0024)\
...
Audit Basis: LUZ codebase, `/Users/juandavidps/Desktop/AXA/beta1.02/luz`, main branch as of 2026-08-17, after ADR-0024 (content encryption) shipped and was confirmed live in production, and after a second self-directed audit pass closed several of v0.2.0's own gaps (memory-level deletion, embeddings activation, Garmin consent, admin MFA, third-party removal path). v0.1.0 was audited against commit `33d367a`; v0.2.0 against `5374be7`. This remains a code-snapshot audit, not a full git-history or infrastructure audit; facts drawn from the live Neon/Vercel/OpenAI dashboards were not directly inspected and remain flagged for verification where relevant.
...
## Revision Note — v0.3.0 (Post-ADR-0024, Second Self-Audit)

v0.2.0 shipped alongside ADR-0024 (`docs/adr/ADR-0024_CONTENT_ENCRYPTION_AND_ACCESS_CONTROL.md`) as a design; between that and this revision, ADR-0024 was actually built, and — after a real production incident where a stale Vercel deployment briefly served ciphertext to a live user, root-caused and fixed the same day — confirmed genuinely live. A second self-directed audit (same discipline, source-verified) then closed several more of v0.2.0's own named gaps. **This is the first revision where several previously-CRITICAL items move to closed, not just re-described.**

**What changed in this pass:**

1. **§12/§17/Unknowns #2 and #5 (plaintext content, plaintext OAuth tokens) — CLOSED.** ADR-0024 is live in production: memory content, belief statements, insight descriptions, concept labels, contradiction text, reasoning conclusions, conversation messages and images, `memory_embeddings.content`, feedback comments, third-party Person/Relationship name and notes, and Auth.js OAuth tokens are all encrypted at rest (AES-256-GCM, `core/security/content-cipher.ts`), with the encryption key held only in the deployed application's runtime environment. **This is not end-to-end encryption** — OpenAI still receives plaintext per message, and Memory/Knowledge engines still read plaintext server-side; see §12 for the precise, non-overclaiming description.
2. **§15/Unknowns #12 (no per-item deletion) — CLOSED.** A real, user-facing "hide" (reversible, LUZ still reasons over it) and "delete" (irreversible, hard `DELETE`) now exist per memory, reachable from `/memories`. Deleting a memory also deletes its `memory_embeddings` row — a gap this same audit pass found and closed (a hard-deleted memory previously left an encrypted duplicate of its content orphaned indefinitely).
3. **§8 embeddings — no longer PLANNED, now CURRENT.** `generate-memory-embedding.ts` calls a fourth distinct OpenAI API surface (`text-embedding-3-small`, alongside chat completions, image understanding, and image generation) and a real semantic-retrieval strategy is wired into live chat context assembly. §8 and §10 updated accordingly — this Policy previously committed to updating before this activation; that update is this revision.
4. **§20/Unknowns #3 (Garmin, no consent capture) — partially closed.** `/garmin` now requires an explicit acknowledgment before the manual-transfer flow is offered, and the import script itself refuses to run without a recorded consent — enforced in code, not only in the UI. Still open: whether this satisfies an Art. 9 basis, and whether the transfer channel itself (email) should be replaced — see Unknowns.
5. **§18/Unknowns #6 (no rights mechanism for third parties) — partially closed.** An admin-only, justification-required, logged path now exists to remove a specific third-party Person record on request. This is not a self-service right (the third party has no account to exercise it from) — it is the most honest mechanism available given that constraint, not a complete answer to the underlying question of whether this data should be minimized instead.
6. **§18/Unknowns #11 (Sentry, no PII scrubbing) — CLOSED.** All three runtime configs now set `sendDefaultPii: false` and a `beforeSend` hook that strips cookies, headers, and non-ID user fields before any event leaves the application.
7. **§18 (admin access control) — MFA added.** `/admin/*` now requires a verified TOTP second factor in addition to the `ADMIN_EMAILS` allowlist, enrollment-gated (no admin page is reachable without it). Break-glass justification + logging (introduced in ADR-0024) is now confirmed to include memories the account holder has hidden from their own view — disclosed explicitly, since "hidden" was never meant to imply "invisible to a diagnostic review," and that distinction was not previously stated anywhere.

**Still not done, named explicitly rather than left silent:** the restricted-role SQL (`core/db/ops/create_restricted_ops_role.sql`) has not been applied to the production database — it requires direct infrastructure access this audit process cannot exercise on its own. AI-provider retention/training terms (Unknowns #1) remain unconfirmed. MFA has no backup/recovery-code flow — loss of the authenticator device today means recovering via direct database access, an accepted trade-off at current admin-count scale, not an oversight.
...
| Third-party (non-user) Person/Relationship records | CURRENT, **updated from v0.2.0** | Mentioning another person in conversation (e.g., "my mother") auto-creates a persistent `Person` record (deduplicated by name) and, where a relationship is described, a `Relationship` record, scoped to the user's LifeGraph; now encrypted at rest, and removable via a logged admin request | third party's name + relationship type (e.g., "mother," "colleague"); the schema's free-text `notes` field exists but is `undefined`/unpopulated on this automatic path | Personal data about a non-consenting third party, with no account and no notice — a removal path now exists, but it is admin-mediated, not self-service, and capture is not yet minimized at the source | 4, 18 | `core/life/services/find-or-create-person.ts:21-46`; `core/life/repositories/drizzle-person.repository.ts` (encryption); `app/api/admin/persons/[id]/delete/route.ts` (removal) |
...
| Memory hide (reversible) and delete (real, hard) | **CURRENT — superseded the old unwired "forget" stage, v0.3.0** | The original `default-forget-stage.ts` (status flag, zero real callers) remains unwired and unused; it was superseded, not activated — the real mechanism is `MemoryRepository.setHiddenFromUser()` (reversible, LUZ still uses the memory) and `MemoryRepository.delete()` (real `DELETE`, now also removes the matching `memory_embeddings` row), both reachable from `/memories` | memory content (deleted, not retained, on real delete) | A policy can now honestly claim users can hide or delete a memory — closes the v0.1.0/v0.2.0 finding that no route reached the old forget code | 7, 15 | `app/api/memories/[id]/hide/route.ts`; `app/api/memories/[id]/delete/route.ts`; `core/memory-engine/repositories/drizzle-memory.repository.ts` |
...
| `memory_embeddings` / pgvector | **CURRENT — updated from v0.2.0** | Real embedding generation (`generate-memory-embedding.ts`, OpenAI `text-embedding-3-small`, a 4th distinct OpenAI API surface) wired into live semantic memory retrieval; a duplicate copy of source content plus the vector are persisted, both encrypted at rest (ADR-0024), both deleted when the source memory is deleted | encrypted content duplicate + embedding vector | Semantic/vector-based profiling of user content now genuinely occurs — must be described as current, not planned | 8, 12, 15 | `core/memory-engine/embedding/generate-memory-embedding.ts`; `core/memory-engine/retrieval/ai-semantic-memory-retrieval-strategy.ts`; `features/chat/services/select-contextual-memories.ts` |
...
| Encrypted content storage (general) | **CURRENT — updated from v0.2.0, ADR-0024** | AES-256-GCM encryption for connector credentials (Gmail/Apple Calendar), Auth.js OAuth tokens, and the substance of stored content — memory content, belief statements, insight descriptions, concept labels, contradiction text, reasoning conclusions, conversation messages/images, `memory_embeddings.content`, feedback comments, third-party Person/Relationship name+notes — via a shared key held only in the application's production runtime, confirmed live end-to-end against real production data (including recovery from a same-day deploy-sequencing incident, root-caused and fixed) | nearly all persisted content | Closes the v0.2.0 CRITICAL plaintext-content and plaintext-OAuth-token findings. **Not** end-to-end encryption — see §12's explicit limitation language; the AI provider and the running application both still process content in the clear | 12, 18 | `core/security/content-cipher.ts`; `core/security/aes-gcm-cipher.ts`; `auth/encrypted-adapter.ts`; `docs/adr/ADR-0024_CONTENT_ENCRYPTION_AND_ACCESS_CONTROL.md` |
| Restricted database role | **PLANNED, written not applied** | Column-level allow-list SQL for a least-privilege ops role, reviewed and ready | — | Until applied, anyone with the primary database credential still has undifferentiated read access to ciphertext (though not to plaintext) across all tables | 12, 18 | `core/db/ops/create_restricted_ops_role.sql` (not yet run against production) |
| Full account deletion | CURRENT, **with one confirmed exception** | Auth-scoped endpoint; transactional hard delete of the LifeGraph (cascades to memories/beliefs/concepts/connections) and the user record; two-step-confirm UI, now also surfaced on the main dashboard, not only in a header link. **Correction**: `events.userId` uses `onDelete: "set null"`, not cascade — sign-in/operational event rows survive deletion with the identifying `userId` stripped, not removed | all personal data tied to the account, except anonymized operational-event rows | Still the strongest deletion claim LUZ can make, but "cannot restore the data" needs to be scoped — it is accurate for identifiable data, not for the fact that some anonymized rows persist | 15 | `app/api/account/delete/route.ts`; `core/account/delete-account.ts:8-9,27-39`; `components/delete-account-button.tsx:14-60`; `core/db/schema/events.ts:82-84` (`onDelete: "set null"`, confirmed) |
| Per-memory hide/delete | **CURRENT — new, closes v0.2.0's largest gap** | Real, user-facing hide (reversible, LUZ still uses it) and delete (irreversible, real `DELETE`, now also removes the corresponding `memory_embeddings` row) reachable from `/memories`; multi-select supported | memory content and its semantic-search duplicate | The single biggest change between what a "memory" product implies and what LUZ could actually deliver, per the v0.2.0 Lawyer Review Notes — substantially closed | 15 | `app/api/memories/[id]/hide/route.ts`; `app/api/memories/[id]/delete/route.ts`; `core/memory-engine/repositories/drizzle-memory.repository.ts` |
| Admin break-glass access control | **CURRENT — new** | `/admin/*` requires a written justification (logged immutably, `admin_access_log`) and a verified TOTP second factor before any decrypted user content renders; includes memories the account holder hid from their own view, disclosed explicitly | — | Closes the v0.2.0 gap where admin access was unaudited and single-factor | 18 | `app/admin/require-mfa.ts`; `core/security/totp.ts`; `core/db/schema/admin-mfa.ts` |
| Third-party record removal | **CURRENT — new, admin-mediated** | A logged, justification-required admin route can remove a specific third party's Person/Relationship record on request | — | Closest available mechanism given the third party has no account; does not address whether capture should be minimized at the source | 4, 18 | `app/api/admin/persons/[id]/delete/route.ts` |
| Garmin consent capture | **CURRENT — new, partial close** | An explicit acknowledgment is required before the wearable-sharing flow is offered, and enforced by the import script itself (refuses without a recorded consent), not only presented in the UI | consent record (metadata, not health data itself) | Narrows, does not fully close, the v0.2.0 CRITICAL Garmin finding — the transfer channel (email) is unchanged | 4, 20 | `core/db/schema/wearable.ts` (`wearableConsents`); `app/api/wearable/consent/route.ts`; `.scratch/import-garmin-export.ts` |
| Sentry error monitoring | CURRENT | Crash/performance monitoring across three runtime configs (server, edge, client); DSN intentionally public (write-only by design); session replay explicitly disabled; **explicit PII-scrubbing configured in all three as of this revision** (`sendDefaultPii: false`, shared `beforeSend` hook stripping cookies/headers/non-ID user fields) | error context, stack traces, 10% trace sample | Closes the v0.2.0 HIGH finding that this relied entirely on SDK defaults | 5, 18 | `instrumentation-client.ts`; `sentry.server.config.ts`; `sentry.edge.config.ts`; `core/observability/sentry-scrub.ts` |
...
2. **(E/L) CLOSED as of v0.3.0** — Memories, beliefs, concepts, conversation text, and uploaded/generated images are now encrypted at rest (ADR-0024, `core/security/content-cipher.ts`, confirmed live in production). Remaining sub-question: `[TECHNICAL FACT TO VERIFY]` whether Neon also provides disk-level encryption as a compensating/defense-in-depth control — not required for this item to close, worth confirming for completeness.
3. **(P/L) PARTIALLY CLOSED as of v0.3.0** — An explicit consent acknowledgment is now required, and enforced by the import process, before Garmin data can be shared (Section 20). Still open: whether this satisfies an Art. 9 basis, and the transfer channel itself (email to an admin inbox) has not been replaced with a secured one.
...
5. **(E) CLOSED as of v0.3.0** — The Auth.js `accounts` table (Google OAuth login tokens) is now encrypted at rest via the same standard as item 2. Was confirmed plaintext as of v0.2.0; ADR-0024 closed this alongside the general content-encryption gap.
6. **(L/P) PARTIALLY CLOSED as of v0.3.0** — Structured, persistent records (name + relationship type) about people the user mentions are now encrypted at rest, and an admin-mediated, logged removal path exists on request. Still open: whether capture should be minimized at the source rather than only remediated after the fact, and whether the removal path should be something other than admin-mediated. `core/life/services/find-or-create-person.ts:21-46`, `find-or-create-relationship.ts:30-67`.
...
11. **(E) CLOSED as of v0.3.0** — All three Sentry runtime configs now set `sendDefaultPii: false` and a shared `beforeSend` hook that strips cookies, headers, and non-ID user fields.
12. **(P/L) CLOSED as of v0.3.0** — Real hide and delete now exist per memory, reachable from the product (`/memories`), not only in unwired backend code.
13. **(P)** The `suppressed` flag is developer-only tooling, not a user control — must not appear in user-facing rights language. (Distinct from the new user-facing "hide," which is a real control — do not conflate the two in future drafting.)
...
29. **(E) NEW, v0.3.0** — Admin MFA (Section 18) has no backup/recovery-code flow. Losing the authenticator device means recovery via direct database access. Acceptable at today's admin-count scale; revisit before adding more admins.
30. **(P) NEW, v0.3.0** — Deleting a memory does not revise beliefs/insights/concepts previously derived from it (Section 15) — the synthesized characterization can outlive its source evidence. Product/Legal should decide whether this needs disclosure treatment beyond what Section 15 already states, or a future remediation mechanism.
31. **(E) NEW, v0.3.0** — The restricted-role SQL for database access (Section 12/18) is written and reviewed but has not been applied to the production database — this requires direct infrastructure action, not a code deploy.
...
**Information about other people.** When you mention another person in conversation — for example, a family member or colleague — LUZ may create a structured, persistent record representing that person within your account, consisting of their name and the type of relationship you described. This happens automatically as part of how LUZ organizes information about your life; the person you mention does not consent to this and is not notified when the record is created. This information is encrypted at rest the same way as the rest of your account content (Section 12). `[CURRENT — new in this revision]` Because that person has no account of their own to exercise a deletion request from, LUZ personnel can remove a specific person's record on request, through the same justification-and-logging process described in Section 18 — the closest mechanism available today, not a self-service right. `[LEGAL/PRODUCT ASSUMPTION — VERIFY]` Whether this practice should also be limited at the point of capture (data minimization), rather than only remediated on request, remains an open Product decision.
...
`[CURRENT — updated from "PLANNED" in v0.2.0]` LUZ represents the content of your memories as numerical vectors ("embeddings") to support meaning-based search, using a dedicated OpenAI embeddings model separate from the model that generates conversational responses. This powers a semantic-retrieval step used when assembling the context for a response — LUZ can surface a relevant memory even when your current message doesn't share exact words with it. The vector itself, and a duplicate copy of the source content stored alongside it for this purpose, are encrypted at rest (Section 12) and are deleted when the source memory is deleted (Section 15).
...
|---|---|---|
| OpenAI | Generates conversational responses; understands images you upload; generates images on request; generates embeddings for semantic memory search — four distinct capabilities/API surfaces, one vendor | CURRENT — primary/active |
...
*In plain terms: the substance of what you say to LUZ — your messages, memories, and what LUZ has concluded about you — is encrypted in our database. A person with direct database access sees unreadable ciphertext, not your words, unless they are also running the application itself.*

LUZ stores data in a PostgreSQL database. `[CURRENT — updated from v0.2.0, ADR-0024 shipped and confirmed live in production]` The substance of stored content — conversation messages and attached/generated images, memory content, belief statements, insight descriptions, concept labels, contradiction descriptions, reasoning conclusions, feedback comments, the semantic-search embedding record described in Section 8, and structured records about third parties you mention (Section 4) — is encrypted at rest (AES-256-GCM), using an encryption key held only in the application's deployed production environment, separate from the database credential itself. Credentials for optional third-party connections (Gmail, Apple Calendar) and the OAuth tokens used for Google Sign-In (Section 17) use the same standard.

`[LEGAL/PRODUCT ASSUMPTION — VERIFY — important limitation, not an oversight]` This is encryption **at rest**, not end-to-end encryption. It protects against someone reading the database or a backup of it directly. It does not, and is not intended to, prevent LUZ's own running application from reading your content — the product's core function (remembering and reasoning about your life, generating a response using a third-party AI model) requires that. Section 10 describes what reaches the AI provider; that is unaffected by this section, since the AI provider necessarily receives content in a readable form to process it. `[TECHNICAL FACT TO VERIFY]` A dedicated, least-privileged database role — intended to be the only credential used for routine operations/debugging queries, distinct from the credential the application itself uses — has been designed and documented but not yet applied to the production database as of this draft.
...
*In plain terms: you can delete your entire account and everything in it, permanently. You can also hide or permanently delete an individual memory on its own, without touching the rest of your account — hiding keeps LUZ's understanding intact but off your own screen; deleting removes the memory and everything derived directly from it, for good.*

**Full account deletion.** LUZ provides a direct, user-initiated way to delete your account. Deleting your account permanently deletes your associated data — including memories, derived insights and beliefs, concepts, conversations, and connected-integration records — together with your account and authentication record. This is an irreversible, hard deletion, not a status change: once completed, LUZ cannot restore the deleted data. *(CURRENT.)* One category of internal operational record — a log entry marking that a sign-in occurred — is not deleted by this process; instead, the entry is stripped of the identifier linking it to you and retained in that anonymized form. This does not restore any of your personal data, but it means "all... data" should not be read as an unqualified claim about every database row. Separately, if you have used LUZ's administrative support tooling and a member of the team has viewed your account content, a record that this occurred (who, when, and the stated reason) is kept regardless of account deletion — this is an internal accountability record about LUZ's own conduct, not your personal data, and Section 18 describes it further.

**Deleting individual items.** `[CURRENT — updated from v0.2.0, this is the single largest change in this revision]` You can hide a memory from your own view (LUZ continues to use it when responding to you — hiding is a personal display preference, not a request that LUZ forget something) or permanently delete it. Deletion is a real, irreversible removal of the memory record, and — closing a gap this audit specifically found — also removes the corresponding semantic-search record described in Section 8, so no duplicate of the deleted content is left behind in that separate table. Deleting a memory does **not** retroactively delete or revise a belief, insight, or concept that was previously derived in part from it; the derived characterization can outlive the specific memory that originally supported it. `[LEGAL/PRODUCT ASSUMPTION — VERIFY]` Whether this residual-derived-data behavior needs its own disclosure treatment, or a future mechanism to also revisit affected derived data, has not been decided.
...
LUZ uses Google Sign-In for authentication, with server-side, database-backed sessions that LUZ can revoke directly (rather than long-lived tokens that persist independent of the server). Information stored for this purpose includes your email address, name, profile image, and the OAuth and session tokens needed to keep you signed in. `[CURRENT — closed in this revision]` These authentication tokens are encrypted at rest, the same standard applied to the credentials for optional connectors such as Gmail and Apple Calendar (Section 12) — this was an open gap as of v0.2.0 and is now closed.

### 18. Security

LUZ applies the following measures today: encryption at rest (AES-256-GCM) for both connector credentials and the substance of stored content described in Section 12; disconnecting an optional connector actively erases the stored secret rather than merely marking it inactive; a deliberately minimal permission scope for Gmail access that cannot read message content; account-scoped API access (for example, account deletion can only be triggered by the authenticated account owner); server-revocable sessions; error monitoring configured without screen-recording and with explicit filtering of cookies, request headers, and non-essential user identifiers before any error report leaves the application; and, for LUZ personnel with administrative access to a user's account content, a requirement to record a written reason before viewing it, an immutable log of every such access, and a second authentication factor beyond the base admin allowlist. `[CURRENT — new in this revision]` That access log includes any content the account holder has hidden from their own view (Section 15) — hiding is a personal display preference, not a restriction on LUZ personnel access, and this Policy states that explicitly rather than leaving it to be inferred. `[LEGAL/PRODUCT ASSUMPTION — VERIFY]` LUZ does not hold, and does not claim, any independent security certification (such as SOC 2 or ISO 27001) as of this draft. `[TECHNICAL FACT TO VERIFY]` A dedicated, restricted database role for routine operational access — distinct from, and narrower than, the credential the application itself uses — has been designed but not yet deployed. A documented incident-response procedure and a completed subprocessor security review remain in progress; this Policy will avoid general assurances ("industry-leading security") that are not backed by a specific, verifiable control.
...
`[UPDATED in this revision — partial close]` As of v0.2.0, LUZ applied no distinct consent flow to wearable data beyond the general protections applied to all account data. `[CURRENT]` A specific, explicit acknowledgment is now required before the wearable-sharing flow is offered — you must affirmatively confirm you understand this involves sending health data by email to LUZ personnel for manual import, and that confirmation is recorded and required before any import can proceed (enforced by the import process itself, not only presented in the interface). `[LEGAL/PRODUCT ASSUMPTION — VERIFY — still open]` Whether this acknowledgment satisfies an Art. 9 explicit-consent basis under GDPR (or equivalent), and whether the underlying transfer channel (email) should be replaced with something more secure before this integration is offered more broadly or in jurisdictions that regulate special-category data, remain for Legal and Product to determine — the consent gap is narrowed, the transfer-channel gap is not yet closed.
...
Ordered by the priority tier from Section 4. Items marked **DONE (v0.3.0)** were closed between this revision and the last; the rest carry over.

**CRITICAL**

1. Obtain and document actual data-retention and model-training terms with OpenAI and Moonshot/Kimi; enable a zero-data-retention option if available at LUZ's account tier. Confirm these terms (or the lack of them) apply equally to the 14+ background/scheduled call sites, not only interactive chat — this list now also includes the embeddings-generation endpoint (Section 8) and the image-generation endpoint (Section 10). **Still the single largest open item in this document.**
2. **DONE (v0.3.0).** Encryption-at-rest implemented and confirmed live in production for memories, beliefs, concepts, conversations, images, and derived data generally (ADR-0024). Remaining sub-item: apply the already-written restricted database role (`core/db/ops/create_restricted_ops_role.sql`) to production — an infrastructure action, not a code change.
3. **DONE (v0.3.0).** Real, user-facing hide/delete per memory shipped (`/memories`). New follow-on, not previously named: deleting a memory does not revise derived beliefs/insights/concepts built from it (Unknowns #30) — decide whether that needs its own product treatment.
4. **PARTIALLY DONE (v0.3.0).** Explicit consent step added and enforced by the import script itself. Still open: replace the manual email transfer channel with something secured before offering this integration more broadly.
5. **DONE (v0.3.0).** Auth.js `accounts` table (Google OAuth tokens) now encrypted via the same pattern as connector credentials, as part of the same ADR-0024 rollout as item 2.

**HIGH**

6. **PARTIALLY DONE (v0.3.0).** An admin-mediated, justification-logged removal path now exists for a specific third party's record. Still open: whether capture should also be minimized at the point of creation, not only remediated afterward.
7. Document, for the legal team, which of the 14+ `getAIProvider()` call sites run outside an active user session (see Section 3 mapping table) so Legal can confirm applicable AI-provider contractual terms cover that usage.
8. **DONE (v0.3.0).** PII-scrubbing configuration added to all three Sentry runtime configs (`beforeSend` hook, `sendDefaultPii: false`).
...
12. **New, v0.3.0.** Admin MFA has no backup/recovery-code flow (Unknowns #29) — decide whether one is needed before adding more admins beyond the current small set.

**MEDIUM**

13. Decide whether `events.userId` should cascade-delete on account removal instead of anonymizing (`onDelete: "set null"`), or formally document anonymization as the intended design — either resolves the Section 15 gap, but it is a product/legal decision, not a default engineering fix.
14. Define a retention schedule (or a documented rationale for indefinite retention) for logs, `events` table rows, and derived Knowledge/Belief/Concept data.
15. Stand up a privacy contact channel (and a DPO, if EU operation requires one) plus a basic data-subject-request intake process, even if manual at first.

**LOW**

16. Treat any future activation of Moonshot/Kimi for live traffic as a mandatory policy-update trigger — do not activate silently.
17. **DONE (v0.3.0), left here for the record.** Embeddings-based/semantic memory search activated; Sections 8 and 10 were revised in this same pass, per the standing instruction this item recorded.
18. Given how quickly this repository ships ("URGENT, live" commits within hours, confirmed a second time this revision by the ADR-0024 deploy incident), treat any future privacy-document review as needing a same-day re-verification pass immediately before external transmission, not a reliance on a dated snapshot audit.
