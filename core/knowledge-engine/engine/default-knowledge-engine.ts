...
import { DefaultInsightConnectStage } from "../relationships/default-insight-connect-stage";
...
export function createKnowledgeEngine(db: Database): KnowledgeEngine {
  ...
  return new DefaultKnowledgeEngine({
    ...
    persist: new DefaultPersistStage(
      insightRepository,
      new DefaultInsightConnectStage(insightRepository),
    ),
  });
}
