...
5. **Timezone real de la persona**: `getCalendarSnapshot`/`getUpcomingEvents` calculan "hoy" en fronteras de día UTC (ver `calendar-timing-helpers.ts`) -- no reciben la zona horaria real de la persona en ningún parámetro hoy. Agregar ese parámetro es aditivo (un `timeZone?: string` más en `CalendarSnapshotOptions`), no un cambio de forma.
