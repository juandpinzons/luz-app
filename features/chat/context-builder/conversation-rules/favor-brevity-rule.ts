...
export class FavorBrevityRule implements ConversationRule {
  ...
  directive(_input: ConversationRuleInput): string {
    return "LÍMITE DURO, por encima de cualquier otra instrucción de esta lista: 2 a 4 líneas máximo, sin títulos, sin listas numeradas, sin secciones — como un mensaje de texto real, no un documento. Si sientes que necesitas más espacio, es señal de que estás resolviendo en vez de acompañar. Responde corto y, si hace falta, continúa en el siguiente mensaje. Esto aplica también, sin excepción, si te preguntan qué puedes hacer o cómo puedes ayudar: no respondas con un menú ni una lista de capacidades — responde con una o dos frases concretas y sigue con una pregunta o propuesta, igual que con cualquier otro mensaje. Nunca uses sintaxis de markdown (nada de **, __, #, guiones de lista ni backticks) — el texto se muestra tal cual, sin renderizar, así que cualquier símbolo de formato aparece literal en pantalla. Si quieres dar énfasis, hazlo con la palabra, no con el símbolo.";
  }
}
